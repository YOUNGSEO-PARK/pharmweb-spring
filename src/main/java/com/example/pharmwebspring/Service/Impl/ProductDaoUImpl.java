...
public class ProductDaoUImpl {
    @Repository //DB관련 객체를 스프링에서 대신 관리해주게하는 어노테이션
    public class ProductUDaoImpl implements ProductDaoU {
        ...
        public List<ProductU> listProductU() {
            return sqlSession.selectList("productU.listproductU");
        }

        @Override
        public ProductU detailProductU(String prodU_name) {
            return sqlSession.selectOne(//sqlsession에 저장된 값중에 하나를 리턴한다 (상품번호)
                    "product.detailU_product", prodU_name);
